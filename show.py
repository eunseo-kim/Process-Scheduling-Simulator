...
class MyApp(QWidget):
    ...
    def add(self):
        ...
        if self.cur_algo != "YOSA":
            ...
            self.proc_list.append(
                Process(proc_name, self.at.value(), self.bt.value(), self.process_count)
            )
        ...
        if self.cur_algo != "YOSA":
            ...
            for Q_proc_idx, Q_process in enumerate(self.history[0][0]):
                self.ready_table.setItem(
                    0, Q_proc_idx, QTableWidgetItem(Q_process.id)
                )
            ...
            for student_idx, student in enumerate(scheduler.students):
                ...
                for subject in range(len(student.best_solo_subjects_grade)):
                    ...
                    self.result_table.setItem(
                        student_idx, 2 + subject * 2, QTableWidgetItem(str(round(student.best_solo_subjects_grade[subject],2)))
                    )
                ...
                if len(student.best_solo_subjects_grade) == 0:
                    for subject_idx in range(len(student.subject_list)):
                        ...
                        self.result_table.setItem(
                            student_idx, 2 + subject_idx * 2, QTableWidgetItem(str(0))
                        )
                self.result_table.setItem(
            ...
            self.result_table.setItem(0, 10, QTableWidgetItem(str(round(scheduler.students[0].best_team_play_grade, 2))))
                ...
                # CPU가 쉬는 도중에는 할당될 프로세스가 없을 가능성 존재
                    if self.history[seconds + 1][1][cpu]:
                        ...
                        self.gantt_table.setItem(
                            cpu, seconds, QTableWidgetItem(self.history[seconds + 1][1][cpu].id)
                        )
